...
The chain and its gates are defined once in `scripts/ship-chain.json`; the section below is generated from it.
...
<!-- SHIP-CHAIN:START — generated by scripts/write-ship-chain-doc.js, do not edit by hand -->

`SHIP-GYM.ps1` is the shipping authority and it reads `scripts/ship-chain.json`.
This section is generated from that same file — edit the JSON, then run
`node scripts/write-ship-chain-doc.js`. `scripts/verify-ship-chain.js` fails the
build if the two ever disagree.

**Normal route — one command, runs everything below plus every gate, and pushes nothing if a gate fails:**

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File C:\Projects\pattayagym\SHIP-GYM.ps1
```

Add `-DryRun` to build and gate without touching git.

### Build — 37 steps, in this order
...
node scripts/annotate-thai-lang.js
node scripts/inject-guide-schema.js
...
- `scripts/apply-design-2026.js` — build-v2.js only regenerates the pages it owns. Without the two sweeps the ~59 static pages revert to the old dark chrome.
- `scripts/polish-design-2026.js` — Fixes the inline-style leftovers a stylesheet cannot reach, and patches the templates that emit them.
- `scripts/annotate-thai-lang.js` — Marks visible Thai runs lang="th" across every shipped page. A sweep, not a source fix, because the Thai-terms guide is written by scripts/write-round62-tier-b.js, which is not in this chain (S2-H3).
- `scripts/inject-guide-schema.js` — The write-*-guides scripts rewrite guides from scratch and drop the FAQPage schema injected earlier. verify-deploy fails without this.
- `scripts/build-press-kit.js` — Last among HTML writers except the entity/date passes: it swaps <main> on /press/ and must see the chrome the sweeps just applied.
- `scripts/normalize-entity-graph.js` — Resolves bare founder Person nodes to @id, and gives any page showing the byline the author markup to match (S2-B1).
- `scripts/update-sitemap-lastmod.js` — Must run last: it hashes the final HTML to decide each page's real change date, then writes both sitemap lastmod and JSON-LD dateModified from that one ledger (S1-4).

### Gates — 13, all must pass or nothing ships

```cmd
node validate.js
node scripts/verify-encoding.js
node scripts/check-no-network-links.js
node scripts/verify-regressions.js
node scripts/verify-ship-chain.js
node scripts/verify-asset-version.js
...
Why each hard gate exists:

- `scripts/verify-encoding.js` — On 2026-07-27 a bad save turned every baht sign and em dash in data.js, build-v2.js and build-discovery.js into codepage garbage and a build propagated it to 306 files. Every other gate stayed green, because mojibake is valid UTF-8.
- `scripts/check-no-network-links.js` — Zero sister-site references in any machine-readable position, and exactly one followed timpaemi.com credit per page. Also a prebuild hook, but SHIP bypasses npm run build, so it is listed here explicitly.
- `scripts/verify-regressions.js` — Proves each defect repaired after the July 2026 audit stays repaired: the JSON-LD sister-domain leak, venue status reconciliation, dateModified integrity, and false first-hand claims.
- `scripts/verify-ship-chain.js` — Fails if the documented chain drifts from this manifest, or if a listed script is missing. The defect S2-F1 described was three documents disagreeing; this is what stops it recurring.
- `scripts/verify-asset-version.js` — Versioned CSS/JS is cached for a year in _headers. This fails the ship if one of those files changed without ASSET_VERSION being bumped, or if any page still references an old version - the only way a year-long cache can hurt you.
- `scripts/verify-deploy.js` — No truncated HTML, NUL bytes, CSP hash drift or sitemap gaps.
- `scripts/verify-design-layer.js` — Proves the 2026 chrome and the live venue count landed on every page - catches the design sweeps falling out of the chain, which no other gate sees.
- `scripts/verify-redirects.js` — The only check comparing _redirects, sitemap.xml and the pages on disk against each other. Stops a retired area x category combo being rebuilt and 301d away the day a venue refills it.

### Run deliberately, not on every ship

- `scripts/check-record-originality.js` — Takes venue ids as arguments and is run deliberately, not on every ship. The July 2026 audit listed its absence from SHIP as part of the chain divergence; it is recorded here as an explicit manual step rather than left as an unexplained gap.

  ```powershell
  $ids = node -e "const {GYMS}=require('./data'); console.log(GYMS.map(x=>x.id).join(' '))"; node scripts/check-record-originality.js @($ids -split ' ')
  ```
...
`SHIP-GYM.ps1` above is the normal route and the only one that runs every gate.
The steps below are the manual fallback, for when it cannot run.

1. Work on branch `redesign-2026-07` (or feature branch → PR → merge to `main`).
2. Bump `ASSET_VERSION` in `build-v2.js` when CSS/JS/fonts change.
3. Run the build and the gates listed above, in that order.
4. Commit **source + generated HTML together**, then push to production:

```cmd
git push origin redesign-2026-07
git tag -f main-pre-round<N> origin/main
git push origin redesign-2026-07:main
...
The 92 `PUSH_ROUND*.cmd` files at the repo root are historical. Every one of them
still contains a live `git push`, none of them runs the current gate set, and
none should be used to ship.
