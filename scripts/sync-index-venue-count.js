...
function patchVenueCount(html) {
  return html
    .replace(/<div class="stat-num">\d+<\/div>(\s*<div class="stat-label">Venues listed)/, `<div class="stat-num">${n}</div>$1`)
    .replace(/\b158-venue\b/gi, `${n}-venue`)
    .replace(/\b215-venue\b/gi, `${n}-venue`)
    .replace(/\b158-entry\b/gi, `${n}-entry`)
    .replace(/\b158\+/g, `${n}+`)
    .replace(/\b158 VENUES\b/g, `${n} VENUES`)
    .replace(/\b215 VENUES\b/g, `${n} VENUES`)
    .replace(/\b158 Venues\b/g, `${n} Venues`)
    .replace(/\b215 Venues\b/g, `${n} Venues`)
    .replace(/\b158 venues\b/g, `${n} venues`)
    .replace(/\b215 venues\b/g, `${n} venues`)
    .replace(/\b215 listings\b/gi, `${n} listings`)
    .replace(/\b215 Pattaya\b/g, `${n} Pattaya`)
    .replace(/\b158 verified\b/gi, `${n} verified`)
    .replace(/\b215 verified\b/gi, `${n} source-checked`)
    .replace(/\b158 (?:hand|source)-checked\b/gi, `${n} source-checked`)
    .replace(/\b215 (?:hand|source)-checked\b/gi, `${n} source-checked`)
    .replace(/\b158 Pattaya sport venues\b/g, `${n} Pattaya sport venues`)
    .replace(/across 158 (?:hand|source)-checked/gi, `across ${n} source-checked`)
