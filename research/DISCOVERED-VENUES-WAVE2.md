# Discovered venues — wave 2 (Thai-language)

Discovery date: **2026-07-20**. This is the approved discovery-only stop point; no
full venue records were created.

## Method and decision rules

The sweep used Thai-script category queries across Pattaya, Jomtien, Naklua,
Pratamnak, Bang Lamung, Sattahip and Sriracha, then repeated the most productive
queries for East Pattaya / Nong Prue, Huai Yai, Bang Saray and Laem Chabang. It
also checked public sports facilities and hotel facilities explicitly offered to
non-residents.

Every new-looking result was checked against all 157 existing `data.js` records.
The comparison used address, phone, coordinates and host venue where available;
name similarity alone was not treated as an identity match. The table deliberately
does **not** repeat every obviously existing venue returned by the searches. It
does include deceptive alternate-name matches, host-venue programs and the closed
Diamond Badminton listing because those are useful cross-check findings.

This is discovery evidence, not the four-source research required for a full
record. A current Maps listing or recent operator activity is sufficient for
`OPEN` here, but hours, public access and location-specific review data still need
full verification. No prices were collected at this stage.

Status vocabulary:

- `OPEN`: current listing/activity supports operation; eligible for full research.
- `UNCLEAR`: plausible identity, but current operation or exact site is not firm.
- `PRE-OPENING`: announced or labelled opening soon, not yet trading.
- `TEMPORARILY-CLOSED` / `CLOSED`: current closure finding.
- `NOT-A-VENUE`: a coach/program at a host site rather than an independent venue.
...
| แอนี่ไทม์ ฟิตเนส อะเกน พัทยา | Anytime Fitness at Again Pattaya | fitness | central-pattaya | [Thai Maps](https://www.google.com/maps/search/?api=1&query=Anytime+Fitness+Again+Pattaya) | YES — `anytime-fitness-pattaya`; existing record includes Again Pattaya | OPEN |
| ไดมอนด์ แบดมินตัน | Diamond Badminton | badminton | central-pattaya | [Thai Maps](https://www.google.com/maps/search/?api=1&query=Diamond+Badminton+Pattaya) | YES — `diamond-badminton`; exact phone and Pattaya 3rd Road address | CLOSED |
| พัทยา เทนนิส แอนด์ แบดมินตัน อินเตอร์คลับ | Pattaya Tennis & Badminton Inter Club | badminton | east-pattaya | [Thai Maps](https://www.google.com/maps/search/?api=1&query=Pattaya+Tennis+Badminton+Inter+Club) | YES — `pattaya-tennis-badminton-inter-club` | OPEN |
| ยูโร แบดมินตัน | Euro Badminton | badminton | east-pattaya | [Thai Maps](https://www.google.com/maps/search/?api=1&query=Euro+Badminton+Pattaya) | YES — `euro-badminton` | OPEN |
| พัทยา พาเดล คลับ | Pattaya Padel Club | padel | east-pattaya | [Thai Maps](https://www.google.com/maps/search/?api=1&query=Pattaya+Padel+Club) | YES — `pattaya-padel-club` | OPEN |
| เพลย์ พาเดล พัทยา | Play Padel Pattaya | padel | pratamnak | [Thai Maps](https://www.google.com/maps/search/?api=1&query=Play+Padel+Pattaya) | YES — `play-padel-pattaya` | OPEN |
| ชาร์คส์ บาสเกตบอล อะคาเดมี่ | Sharks Basketball Academy | kids-sports | east-pattaya | [Thai Maps](https://www.google.com/maps/search/?api=1&query=Sharks+Basketball+Academy+Pattaya) | YES — program at `planet-football-pattaya`, not an independent venue | NOT-A-VENUE |

## Counts and disposition

There are **79 candidate/check rows**: **69** are identities not represented in
the 157 records, while **10** resolve to an existing record or its host venue.
Of the 69 uncovered identities, **63 are currently open enough to move into the
four-source research queue**. Six are held back: three `UNCLEAR`, one
`PRE-OPENING`, one `TEMPORARILY-CLOSED`, and one `NOT-A-VENUE`.

| category | candidate/check rows | not already covered | open research queue |
|---|---:|---:|---:|
| fitness | 22 | 21 | 20 |
| badminton | 17 | 14 | 12 |
| martial-arts | 6 | 6 | 6 |
...
| swimming | 8 | 7 | 6 |
| yoga | 3 | 2 | 2 |
| hotel-day-pass | 1 | 1 | 1 |
| kids-sports | 1 | 0 | 0 |
| **Total** | **79** | **69** | **63** |

The uncovered identities by area are:

| area | uncovered identities | open research queue |
|---|---:|---:|
| east-pattaya | 12 | 12 |
| sriracha | 12 | 11 |
| jomtien | 8 | 8 |
| pattaya-area-unverified | 8 | 6 |
| bang-saray | 5 | 5 |
| laem-chabang | 4 | 3 |
| huai-yai | 4 | 3 |
| na-jomtien | 4 | 3 |
...
Area attribution for four martial-arts listings and four other listings remains
intentionally unverified rather than guessed.

### Already-covered and alternate-name findings

Ten rows did not survive the existing-record/host check. **Four are meaningful
Thai/English or alternate-brand identity traps** rather than straightforward same-
name matches:

1. G Swim Academy / IG Center = `manta-kids-pattaya` (same address and site).
2. the generic Thai Maps listing “Yoga” = `yoga-pattaya-studio` (same address).
3. IP Soccer Club = a host location already represented by
   `fast-pro-football-academy`.
4. Anytime Fitness at Again Pattaya = a location already included in
   `anytime-fitness-pattaya`.

The other six are four straightforward same-name venue matches (Inter Club,
Euro Badminton, Pattaya Padel and Play Padel), one closed exact
match (Diamond Badminton), and one non-independent program at an existing host
(Sharks Basketball at Planet Football). Diamond's current closed flag should be
handled as a separate maintenance task; this mission did not edit `data.js`.

## Which Thai queries changed the result

| Thai query family | English crawl miss exposed |
|---|---|
| `สนามแบดมินตัน พัทยา`, then `… ห้วยใหญ่`, `… ศรีราชา`, `… แหลมฉบัง` | The clearest gap: neighbourhood courts, Thai directories and a large Sriracha cluster. |
| `เทควันโด พัทยา` | Six current Thai martial-arts academies that generic “gym” searches did not surface. |
| `สอนว่ายน้ำ พัทยา` | Swim schools, local pool clubs and the municipal School 11 competition pool. |
| `ค่ายมวยไทย พัทยา`, `ยิมมวย พัทยา`, plus Bang Saray | Three local camps/branches plus Eagle Gym, none represented in the 157. |
| `สนามฟุตซอล พัทยา` and Nong Prue facility searches | Four bookable football identities, a kids academy and public stadiums. |
| `ฟิตเนส จอมเทียน`, `… ห้วยใหญ่`, `… บางเสร่`, `… ศรีราชา`, `… แหลมฉบัง` | The single largest numerical group; the regional modifiers mattered more than Thai wording alone. |
| `พาเดล พัทยา` | Two already-open post-April clubs plus two not ready for records. Chilli's May 2026 opening explains why the April crawl missed it. |
| `โยคะ พัทยา`, then Huai Yai/Jomtien | Only two new studios; much lower yield than badminton, fitness or martial arts. |

## Searches with little or nothing new

- Yoga produced only two uncovered studios and one already-covered location.
- The hotel/condo public-access sweep produced only one clean new offer: Terra
  Nara's pool day pass. Other results were guest-only, stale, aggregator-only, or
  already represented (for example Hard Rock and Fitz Club).
- No new independent basketball venue survived: Sharks Basketball is a program at
  the already-covered Planet Football site.
- The Thai swimming sweep did not produce a new dive operator or general tourist
  watersports operator; its new value was swim instruction and local pools.

These are bounded claims about the queries run, not claims that no such venue can
exist.

## Assessment and next batch

The hypothesis **holds**. The 157-venue directory is materially incomplete for the
actual Thai-patronised sports market. Sixty-three currently open, uncovered
identities is too large to dismiss as search noise, even before held-back listings
are resolved. Thai-script searching was especially valuable for badminton,
taekwondo, swimming instruction and municipal facilities; geographic modifiers
were decisive for Sriracha, East Pattaya, Bang Saray and Laem Chabang.

There is an important scope choice before records are commissioned. **Twenty-five of
the 69 uncovered identities are in Sriracha, Laem Chabang, Bang Phra or outside a
confident Pattaya-area assignment.** If the directory remains limited to the six
current Pattaya area pages, those should not silently enter the same batch. Even
after setting them aside, the Pattaya/Jomtien/Nong Prue/Huai Yai/Bang Saray/Naklua/
Sattahip side still contains a substantial research queue.

Recommended first eight after review, selected for current status, distinct
identity and evidence quality: **Chilli Padel Club, Prime Padel Pattaya, JP
Badminton, Badminton-Khaonoi, Swimming Kids Pattaya, Baby Shark Swim School, James
GYM and MAVINN Muay Thai**. Full records should still wait for approval and must
meet the four-independent-source rule.
